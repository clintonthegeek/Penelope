...
Run: `cmake --build build --target PenelopeCore -j$(nproc)`
...
- Modify: `src/CMakeLists.txt:22-25` (add new files to PenelopeCore)
...
#ifndef PENELOPE_WEBVIEWRENDERER_H
#define PENELOPE_WEBVIEWRENDERER_H
...
#endif // PENELOPE_WEBVIEWRENDERER_H
```
...
**Files:**
- Modify: `src/app/penelope.kcfg:49-64` (add entry to Rendering group)
...
Run: `cmake --build build --target PenelopeCore -j$(nproc)`
Expected: `PenelopeSettings::self()->useWebView()` and `setUseWebView()` now available.
...
Inside the `if (PenelopeSettings::self()->usePdfRenderer())` branch (line 1058),
...
Run: `cmake --build build -j$(nproc) && ./build/bin/penelope`
